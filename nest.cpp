...
int main(int c,char**v){
 ...
 for(int i=0;i<R;i++){
   ...
   for(int j=0;j<R;j++){
     ...
     bool contains=true; for(auto&p:rooms[i]) if(!rooms[j].count(p)){contains=false;break;}
     if(!contains)continue;
     bool exitsInside=true; for(auto&e:exits[i]) if(!rooms[j].count(e)){exitsInside=false;break;}
     if(exitsInside){ if(best<0||rooms[j].size()<rooms[best].size()) best=j; }
   }
   parent[i]=best;
 }
 // combined: contains 2+ disjoint rooms
 for(int j=0;j<R;j++){
   ...
   for(size_t a=0;a<contained.size()&&!combined[j];a++)
     for(size_t b=a+1;b<contained.size()&&!combined[j];b++){
       bool overlap=false;
       for(auto&p:rooms[contained[a]]) if(rooms[contained[b]].count(p)){overlap=true;break;}
       if(!overlap) combined[j]=true;
     }
 }
 // clear combined as parents
 for(int i=0;i<R;i++) if(parent[i]>=0 && combined[parent[i]]) parent[i]=-1;
 // depth = 1 + chain of true parents
